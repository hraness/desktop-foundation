...
export type RunnerEvent =
  ...
  | { version: 1; type: 'already-running' | 'stopped' | 'validated' }
...
export function assertAppId(id: string): void {
  if (!/^[a-zA-Z0-9][a-zA-Z0-9._-]{0,127}$/.test(id)) throw new Error('invalid-app-id');
}
...
export function parseRunnerEvent(line: string): RunnerEvent {
  ...
  const keys: Record<string, string[]> = { ready: ['version','type','pid','platform'], action: ['version','type','id','revision'], error: ['version','type','code'], stopped: ['version','type'], 'already-running': ['version','type'], validated: ['version','type'] };
